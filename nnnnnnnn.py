...
class MyTestCase(unittest.TestCase):
    ...
    def test_post_get(self):
        ...
        app.test_client().post('/AddMessage/', json=self.data)
        response = app.test_client().post('/AddMessage/', json=self.data2)
        ...
        #assert str(response.data)[2] == str(length)
        # result = str(response.body)
        # assert int(len(result) / length+1) == length + 1
        # assert int(len(result)/len("<main.Message object at 0x04B086A0>, ")) == length+1
    ...
    def test_delete(self):
        ...
        self.length = myApp.get_len()
        response = app.test_client().post('/DeleteMessage/?message_id=mmmm')
        assert response==" "
        string = str(response.data)
        res = string.split("'")[1]
...
#(c.delete('http://127.0.0.1:5000/DeleteMessage/?message_id=rrr'))
# c.post('http://127.0.0.1:5000/AddMessage/', json=data)
# c.post('http://127.0.0.1:5000/AddMessage/', json=data2)
# with flask_server.app.test_client() as c:
#     c.delete('http://127.0.0.1:5000/DeleteMessage/?message_id=bbbb')